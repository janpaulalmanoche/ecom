...
Route::group(['middleware' => ['frontlogin']], function () {
    ...
    Route::match(['get', 'post'], '/select-payment', 'CartController@select_payment');
    ...
    //generate a code by submting get request to unionbank
    Route::get('generate-code','OrderController@generate_code');

    //the generatecode request will throw a code to our callback route
    Route::get('oauth2/callback',"OrderController@call_back_function");

    //from the callback route we will redirect to this route passing the code and making post request
    //to union bank to get an access token
    Route::get('request-online-access-token/{token}',"OrderController@customer_request_online_access_token");
    ...
    Route::get('token2',function(){
        ...
        Session::forget('access_token_union');
        dd(Session::get('access_token_union'));
    });
    

Route::get('sandbox-account','PaymentController@make_sanbox_account');
Route::get('sandbox-account-balance','PaymentController@inquire_sandbox_balance');
...
Route::group(['middleware' => ['adminlogin']], function () { //this is how to put admin protection in laravel.middleware method
    ...
    //reseller reports. profit
    Route::resource('/admin/reseller/report', 'AdminResellerReport');
});
// ending tag admin middlewware
