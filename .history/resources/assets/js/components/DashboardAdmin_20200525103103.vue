    ...
    <ul class="recent-posts" v-for="orders in new_orders" :key="orders.id">
        <li>
          ...
          <div class="article-post">
            ...
            <br />
            <p>
             <a href="#" @click="view_order_detail(orders.id)"> <span style="color:green">New Order Reference Number ( # {{orders.refference_number}} )</span> </a>
            </p>
          </div>
          <a href>
...
// this.mounted = !this.mounted
  ...
  methods:{
      ...
      new_orders_fetch(){
        //   alert('haha')
          axios.get('/api/admin/new-products').then((r)=>{
              this.new_orders = r.data
          })
      },
      view_order_detail(id){
        // alert('test')
        window.location.assign('admin/view-order/1' + id)
      }
  }
};
</script>
