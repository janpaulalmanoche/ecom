...
//            echo"<pre>"; print_r($data);die;
        ...
        $userCart= Cart::with('cartprod')->where(['session_id'=>$session_id])->get();
        ...
        return view('users.checkout')->with(compact('userCart','user_details','provinces',
...
//        }
        ...
        $getSessionID =  $session_id = Session::get('session_id');
        $countProductsinCart = DB::table('carts')->where('session_id' ,$getSessionID)->get()->count();
        return view('products.order_review')->with(compact('user_details','provinces','userCart','countProductsinCart'));
...
//            if(empty($data['total'])){
//                $data['total']= '';
//            } //revmoew eerror when you dont have value

            $pin = mt_rand(1000000,9999999);
            $refferenceNO = str_shuffle($pin);
//            echo"<pre>";print_r($refferenceNO);die;
...
//            $userCart=DB::table('cart')->where(['session_id'=>$session_id])->get();
            ...
            $order_id = DB::getPdo()->lastInsertId(); // to get the last insert id
...
//            $cartProducts = DB::table('cart')->where(['user_email'=>$user_email])->get(); //when we use this it wil get all all the products added to acart using the email even on the pasdt
            $cartProducts = Cart::with('cartprod')->where(['session_id'=>$session_id])->get();//**
            ...
            foreach($cartProducts as $pro){//we rename the cartProducts as pro  //**
                    ...
                    $cartPro ->product_id = $pro->product_id;//we call $pro->product_id to get the id from the cart
...
//
//            }
            DB::table('carts')->where('user_email',$user_email)->delete();//after checkout delete the record where useremail = the auth user email
...
//            DB::table('products')->where('id','=',$order_productS->product_id)->decrement('stocks', $order_productS->quantity);
        ...
        $getSessionID =  $session_id = Session::get('session_id');
        $countProductsinCart = DB::table('carts')->where('session_id' ,$getSessionID)->get()->count();
        return view('users.thanks')->with(compact('cartProducts','countProductsinCart'));
    }
...
//        echo "<pre>"; print_r($countORD);die;

        //we include here our one to many relationship of our order to order products(ordersz)
//With relationship declare ordersz, You can access the other table by using relation ship,

//        foreach($orders as $try){
//            $product_idd = $try->product_id;
//
//            $product_name= Product::where('id',$product_idd)->get();
//        }
...
//        echo "<pre> "; print_r ($orders);die;
        ...
        return view('users.order_history')->with(compact('orders','countORD2',
                'orders2','product_name','try','countProductsinCart','countORD','mytime'));
    }
    ...
    public function PaidOrderHistory(){
        ...
        return view('users.paidorders')->with(compact('orders','product_name','try','countProductsinCart','countORD'));
    }
